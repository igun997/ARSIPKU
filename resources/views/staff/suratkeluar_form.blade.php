...
<script type="text/javascript">
  $(document).ready(function() {
    async function read_jenis(){
      $url = "{{route("staff.api.suratkeluar.jenis.list",1)}}";
    ...
    $("#jenis_id").on('change', async function(event) {
      ...
      id = $(this).val();
      if (id != "") {
        getid = await $.get("{{route("staff.api.suratkeluar.jenis.get")}}/"+id).then();
        counter = await $.get("{{route("staff.api.suratkeluar.api_countsurat")}}?id="+id).then();
